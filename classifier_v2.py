...
def build_classification_prompt(finding: dict, business_context: Optional[dict] = None,
    ...
    return f"""You are a security analyst performing vulnerability triage in a controlled lab environment.
...
def _validate_classification_schema(classification: dict) -> tuple[bool, list[str]]:
    ...
    errors = []
    required_fields = ['severity_bucket', 'attack_vector', 'vuln_component', 'exploit_notes', 'automation_candidate', 'llm_confidence']
    ...
    if 'severity_bucket' in classification:
        ...
        if classification['severity_bucket'] not in valid_severities:
            errors.append(f"Invalid severity_bucket '{classification['severity_bucket']}', must be one of {valid_severities}")
    ...
    if 'attack_vector' in classification:
        ...
        if classification['attack_vector'] not in valid_vectors:
            errors.append(f"Invalid attack_vector '{classification['attack_vector']}', must be one of {valid_vectors}")

    # Validate types
    if 'automation_candidate' in classification and not isinstance(classification['automation_candidate'], bool):
        errors.append(f"automation_candidate must be boolean, got {type(classification['automation_candidate']).__name__}")

    if 'llm_confidence' in classification:
        confidence = classification['llm_confidence']
        if not isinstance(confidence, (int, float)):
            errors.append(f"llm_confidence must be numeric, got {type(confidence).__name__}")
        elif not (0.0 <= confidence <= 1.0):
            errors.append(f"llm_confidence must be in range [0.0, 1.0], got {confidence}")

    # Validate string fields
    for field in ['vuln_component', 'exploit_notes']:
...
def _fix_classification_schema(classification: dict, finding: dict) -> dict:
    ...
    if 'severity_bucket' not in fixed or not fixed['severity_bucket']:
        ...
        severity_map = {'low': 'Low', 'medium': 'Medium', 'high': 'High', 'critical': 'Critical', 'none': 'None', 'info': 'None', 'informational': 'None'}
        fixed['severity_bucket'] = severity_map.get(str(fixed['severity_bucket']).strip().lower(), fixed['severity_bucket'])

    # Fix attack_vector
    if 'attack_vector' not in fixed or not fixed['attack_vector']:
        fixed['attack_vector'] = 'Network'
    else:
        vector_map = {'network': 'Network', 'adjacent': 'Adjacent', 'local': 'Local', 'physical': 'Physical', 'remote': 'Network', 'local network': 'Adjacent'}
        fixed['attack_vector'] = vector_map.get(str(fixed['attack_vector']).strip().lower(), fixed['attack_vector'])
    ...
    if 'automation_candidate' not in fixed:
        fixed['automation_candidate'] = False
    else:
...
def _call_classification_api(finding: dict, provider: str, api_key: str, model: str,
                            base_url: str, business_context: Optional[dict],
                            few_shot_examples: Optional[str]) -> dict:
    """
    Unified LLM classification caller supporting OpenAI, Gemini, and compatible providers.
...
def _get_provider_config(provider: str, model: Optional[str]) -> tuple[str, str]:
    """Get model and base URL for provider."""
    model_defaults = {
        "github": "gpt-4o-mini",
    ...
    base_url_map = {
        ...
        "local": os.environ.get('LOCAL_OPENAI_BASE_URL', 'http://localhost:11434/v1'),
        "openai": "https://api.openai.com/v1"
    }

    final_model = model or model_defaults.get(provider, "gpt-4o-mini")
    base_url = base_url_map.get(provider, "https://api.openai.com/v1")

    return (final_model, base_url)
...
def _is_transient_error(error_msg: str) -> bool:
    ...
    return any(x in error_msg for x in [
        "503", "UNAVAILABLE", "overloaded", "500", "502", "504", "timeout", "Timeout",
        "Empty response from LLM - will retry", "Connection", "connection",
        "No complete JSON object found", "Invalid JSON in response", "Unterminated string"
    ])
...
def classify_single(finding: dict, provider: str = "auto", api_key: Optional[str] = None,
                    model: Optional[str] = None, business_context: Optional[dict] = None,
                    few_shot_examples: Optional[str] = None, max_retries: int = 3,
    ...
    actual_provider, detected_key = _detect_provider_and_key(provider)
    api_key = api_key or detected_key

    if not api_key and actual_provider not in ["local"]:
        raise RuntimeError(f"{actual_provider.upper()}_TOKEN/API_KEY not set")

    # Get model and base URL
    final_model, base_url = _get_provider_config(actual_provider, model)
    ...
    for attempt in range(max_retries + 1):
        try:
            classification = _call_classification_api(
                finding, actual_provider, api_key or "local", final_model,
                base_url, business_context, few_shot_examples
            )
...
def classify_findings(findings: list[dict], provider: str = "auto", model: Optional[str] = None,
                     business_context: Optional[dict] = None, enable_few_shot: bool = True,
    ...
    Args:
        ...
        model: Model name (provider-specific)
        business_context: Optional business rules and environment context
        enable_few_shot: Enable dynamic few-shot example selection
        metrics: Optional ClassificationMetrics instance
    ...
    actual_provider, _ = _detect_provider_and_key(provider)
    ...
    provider_names = {"github": "GitHub Models", "gemini": "Google Gemini", "openai": "OpenAI", "local": "Local (Ollama/LM Studio)"}
    print(f"[*] Classifying with {provider_names.get(actual_provider, actual_provider)}", file=sys.stderr)
    ...
    for i, finding in enumerate(findings, 1):
        ...
        try:
            classified = classify_single(finding, provider=actual_provider, model=model,
                                       business_context=business_context, few_shot_examples=few_shot_text)
    ...
    # Mark metrics as printed to avoid duplicate printing
    if metrics and 'metrics_printed' not in dir(metrics):
        metrics.metrics_printed = True
